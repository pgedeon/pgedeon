## Hi, I'm Peter 👋

I build tools that help people make better decisions.

### 🔧 What I'm building
- **[3DPUT](https://3dput.com)** — The most comprehensive 3D printing filament settings database and printer comparison tool on the web. Find optimal print settings for any printer and filament combo.
- **[Sailing Yachts Database](https://info.sailboats.fr)** — Interactive sailing yacht comparison tool with specs from 50+ manufacturers worldwide.
- **[Sailboats.fr](https://sailboats.fr)** — French sailing guides, equipment reviews, and destination itineraries.

### 📊 What I know
- 3D printing (FDM, SLA, resin) & filament optimization
- Full-stack web development (Next.js, WordPress, PostgreSQL)
- SEO & content strategy
- Sailing & marine equipment

### 🔗 Find me
- 🌐 [3dput.com](https://3dput.com) — 3D Printing Tools & Comparisons
- ⛵ [sailboats.fr](https://sailboats.fr) — Guides Voile en Français
- 🎨 [null.pictures](https://null.pictures) — Art & Print Shop
- 🛒 [Etsy Shop](https://nullpictures.etsy.com) — Handmade Linocut Prints

---
*"Build tools that you'd actually use yourself."*
